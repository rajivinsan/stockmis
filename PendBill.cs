...
namespace GST
{
    ...
    public partial class PendBill : Form
    {
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            {
                 qry = "SELECT  customer.CNAME as [Cutomer Name],Sum(BILL.GTotal) AS [Bill Amount], IIF(ISNULL(Sum(payments.Paid)),0,Sum(payments.Paid)) AS [Amount Paid], [Bill Amount]-[Amount Paid] AS Balance FROM (BILL LEFT JOIN payments ON BILL.CID = payments.CID) LEFT JOIN customer ON BILL.CID = customer.CID GROUP BY customer.CNAME, BILL.cid, BILL.CID";
            }
            else
            {
               qry = "SELECT  customer.CNAME as [Cutomer Name],Sum(BILL.GTotal) AS [Bill Amount], IIF(ISNULL(Sum(payments.Paid)),0,Sum(payments.Paid)) AS [Amount Paid], [Bill Amount]-[Amount Paid] AS Balance FROM (BILL LEFT JOIN payments ON BILL.CID = payments.CID) LEFT JOIN customer ON BILL.CID = customer.CID   where BILL.bdate>=#" + dateTimePicker1.Value.ToString("yyyy/MM/dd") + "# and BILL.bdate<=#" + dateTimePicker2.Value.ToString("yyyy/MM/dd") + "# GROUP BY customer.CNAME, BILL.cid, BILL.CID;";
            }
